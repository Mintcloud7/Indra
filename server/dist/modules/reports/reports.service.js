...
class ReportsService {
    async workOrderReport(filters = {}) {
       ...
       ORDER BY count DESC`, params);
        ...
        const workOrdersResult = db.exec(`SELECT wo.woNumber, wo.title, wo.priority, wo.status, wo.createdAt, wo.closedAt,
    ...
    async maintenanceCostReport(filters = {}) {
        const db = await (0, connection_1.getDb)();
        let where = "WHERE wo.status = 'CLOSED'";
        const params = [];
        if (filters.startDate) {
            where += ` AND wo.closedAt >= ?`;
            params.push(filters.startDate);
        }
        if (filters.endDate) {
            where += ` AND wo.closedAt <= ?`;
        ...
        const totalsResult = db.exec(`SELECT COALESCE(SUM(wsp.totalCost), 0) as "totalCost",
        COUNT(DISTINCT wo.id) as "woCount"
       FROM work_orders wo
       JOIN work_order_spare_parts wsp ON wsp.woId = wo.id
       ${where}`, params);
        const totals = formatRow(totalsResult);
        const byAssetResult = db.exec(`SELECT a.assetName, a.assetCode,
        COUNT(DISTINCT wo.id) as "woCount",
       ...
       ${where}
       GROUP BY wo.assetId, a.assetName, a.assetCode
       ORDER BY "totalCost" DESC`, params);
        const byAsset = formatRows(byAssetResult);
        const byMonthResult = db.exec(`SELECT strftime('%Y-%m', wo.closedAt) as month,
        COUNT(DISTINCT wo.id) as "woCount",
        COALESCE(SUM(wsp.totalCost), 0) as "totalCost"
       FROM work_orders wo
       JOIN work_order_spare_parts wsp ON wsp.woId = wo.id
       ...
       ORDER BY month ASC`, params);
        ...
        const byItemResult = db.exec(`SELECT sp.itemCode, sp.itemName,
       ...
       ORDER BY "totalCost" DESC`, params);
        ...
        const byWorkOrderResult = db.exec(`SELECT wo.id, wo.woNumber, wo.title, wo.closedAt,
        a.assetName, a.assetCode,
        u.name as assignedToName,
        COALESCE(SUM(wsp.totalCost), 0) as "totalCost",
       ...
       ORDER BY "totalCost" DESC`, params);
        ...
        return {
            ...
            trend: byMonth.map((m) => ({ month: m.month, cost: m.totalCost })),
            byAsset,
            byMonth,
            byItem,
            byWorkOrder
    ...
    async sparePartUsageReport(filters = {}) {
        const db = await (0, connection_1.getDb)();
        ...
        const byItemResult = db.exec(`SELECT sp.itemCode, sp.itemName, sp.unit,
       ...
       ORDER BY "totalQuantity" DESC`, params);
        const byItem = formatRows(byItemResult);
        const byAssetResult = db.exec(`SELECT a.assetName, a.assetCode,
        COUNT(DISTINCT wsp.woId) as "woCount",
        SUM(wsp.usedQuantity) as "totalQuantity",
       ...
       ORDER BY month ASC`, params);
        const byMonth = formatRows(byMonthResult);
        const totalsResult = db.exec(`SELECT SUM(wsp.usedQuantity) as "totalQuantity",
        COALESCE(SUM(wsp.totalCost), 0) as "totalCost"
       FROM work_order_spare_parts wsp
       JOIN work_orders wo ON wsp.woId = wo.id
       ${where}`, params);
        ...
        return {
            ...
            totalCost: totals?.totalCost || 0,
            data: byItem.map((r) => ({ name: r.itemName, quantity: r.totalQuantity })),
            byItem,
            byAsset,
            byMonth
        };
    ...
    async mttr(filters = {}) {
        ...
        const overallResult = db.exec(`SELECT 
        COUNT(*) as count,
        AVG((julianday(wo.closedAt) - julianday(wo.startedAt)) * 24) as "avgHours"
       FROM work_orders wo ${where}`, params);
        const overall = formatRow(overallResult);
        const byAssetRowsResult = db.exec(`SELECT wo.assetId, a.assetName, a.assetCode,
        COUNT(*) as count,
        AVG((julianday(wo.closedAt) - julianday(wo.startedAt)) * 24) as "avgHours"
       FROM work_orders wo
       LEFT JOIN assets a ON wo.assetId = a.id
       ...
       GROUP BY month ORDER BY month ASC`, params);
        const monthlyTrend = formatRows(monthlyResult).map((r) => ({
            month: r.month,
    ...
    async mtbf(filters = {}) {
        ...
        const assetRowsResult = db.exec(`SELECT assetId FROM work_orders wo ${where} GROUP BY assetId`, params);
        const assetRows = formatRows(assetRowsResult);
        const assetIds = assetRows.map((row) => row.assetId);
        const byAsset = [];
        for (const assetId of assetIds) {
            const woResult = db.exec(`SELECT wo.id, wo.createdAt
         FROM work_orders wo
         WHERE wo.assetId = ? AND wo.status = 'CLOSED'
         ORDER BY wo.createdAt ASC`, [assetId]);
            ...
            const timestamps = woRows.map((row) => new Date(row.createdAt).getTime());
            const gaps = [];
        ...
        for (const assetId of assetIds) {
            const woResult = db.exec(`SELECT wo.id, wo.assetId, wo.createdAt
         FROM work_orders wo
         WHERE wo.assetId = ? AND wo.status = 'CLOSED'
         ORDER BY wo.createdAt ASC`, [assetId]);
            const woRows = formatRows(woResult);
            ...
            for (let i = 1; i < woRows.length; i++) {
                ...
                if (!monthlyGaps.has(month))
                    monthlyGaps.set(month, []);
    ...
    async assetHistory(assetId) {
        ...
        const workOrdersResult = db.exec(`SELECT wo.woNumber, wo.title, wo.priority, wo.status, wo.createdAt, wo.closedAt, wo.rootCause,
       ...
       ORDER BY wo.createdAt DESC`, [assetId]);
        const workOrders = formatRows(workOrdersResult);
        const pmResult = db.exec(`SELECT pm.title, pm.frequency, pm.nextDueDate, pm.status, pm.createdAt,
        u.name as assignedToName
       FROM preventive_maintenance pm
       LEFT JOIN users u ON pm.assignedToId = u.id
       WHERE pm.assetId = ?
       ORDER BY pm.createdAt DESC`, [assetId]);
        const preventiveMaintenance = formatRows(pmResult);
        const invResult = db.exec(`SELECT it.*, sp.itemCode, sp.itemName, w.name as warehouseName
       ...
       ORDER BY it.createdAt DESC`, [assetId]);
