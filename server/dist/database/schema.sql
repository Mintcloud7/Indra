...
CREATE TABLE IF NOT EXISTS asset_meter_readings (
  id TEXT PRIMARY KEY,
  meterId TEXT NOT NULL REFERENCES asset_meters(id) ON DELETE CASCADE,
  assetId TEXT NOT NULL REFERENCES assets(id) ON DELETE CASCADE,
  value REAL NOT NULL,
...
CREATE TABLE IF NOT EXISTS work_orders (
  ...
  supervisorId TEXT REFERENCES users(id),
...
CREATE TABLE IF NOT EXISTS work_order_status_history (
  id TEXT PRIMARY KEY,
  woId TEXT NOT NULL REFERENCES work_orders(id) ON DELETE CASCADE,
  fromStatus TEXT,
...
CREATE TABLE IF NOT EXISTS work_order_checklists (
  ...
  completedBy TEXT REFERENCES users(id),
  completedAt TEXT,
  notes TEXT
);
...
CREATE TABLE IF NOT EXISTS work_order_spare_parts (
  id TEXT PRIMARY KEY,
  woId TEXT NOT NULL REFERENCES work_orders(id) ON DELETE CASCADE,
  itemId TEXT NOT NULL REFERENCES spare_parts(id),
...
CREATE TABLE IF NOT EXISTS pm_checklists (
  id TEXT PRIMARY KEY,
  pmId TEXT NOT NULL REFERENCES preventive_maintenance(id) ON DELETE CASCADE,
  title TEXT NOT NULL,
  description TEXT,
  required INTEGER DEFAULT 1,
  createdAt TEXT DEFAULT (datetime('now'))
...
CREATE TABLE IF NOT EXISTS pm_wos (
  id TEXT PRIMARY KEY,
  pmId TEXT NOT NULL REFERENCES preventive_maintenance(id) ON DELETE CASCADE,
  woId TEXT NOT NULL REFERENCES work_orders(id) ON DELETE CASCADE,
  createdAt TEXT DEFAULT (datetime('now')),
...
CREATE TABLE IF NOT EXISTS purchase_requisitions (
  id TEXT PRIMARY KEY,
  itemId TEXT REFERENCES spare_parts(id),
  customItemName TEXT,
  customItemCode TEXT,
  quantity REAL NOT NULL,
  unit TEXT,
...
CREATE TABLE IF NOT EXISTS log_book_items (
  id TEXT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS log_book_spare_parts (
  id TEXT PRIMARY KEY,
  logBookId TEXT NOT NULL REFERENCES log_books(id) ON DELETE CASCADE,
  logBookItemId TEXT REFERENCES log_book_items(id) ON DELETE SET NULL,
  sparePartId TEXT NOT NULL REFERENCES spare_parts(id),
