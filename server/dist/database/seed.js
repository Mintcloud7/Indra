...
async function seed() {
    ...
    for (const u of userData) {
        const id = (0, utils_1.generateId)();
        userIds[u.email] = id;
        db.run('INSERT INTO users (id, username, email, password, name, phone, isActive, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)', [id, u.username, u.email, password, u.name, '081234567890', now, now]);
    ...
    for (const m of meters) {
        const id = (0, utils_1.generateId)();
        meterIds.push(id);
        db.run('INSERT INTO asset_meters (id, assetId, meterType, unit, createdAt) VALUES (?, ?, ?, ?, ?)', [id, assetIds[m.assetIdx], m.type, m.unit, now]);
        db.run('INSERT INTO asset_meter_readings (id, meterId, assetId, value, readingDate, recordedBy) VALUES (?, ?, ?, ?, ?, ?)', [(0, utils_1.generateId)(), id, assetIds[m.assetIdx], Math.floor(Math.random() * 5000) + 1000, now, userIds['technician@example.com']]);
    }
    // ========== WORK ORDERS ==========
    const woData = [
    ...
    for (let i = 0; i < woData.length; i++) {
        ...
        db.run(`INSERT INTO work_orders (id, woNumber, title, description, assetId, location, reportedById, supervisorId, assignedToId, priority, status, createdAt, problemDescription, rootCause, resolution, startedAt, closedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [id, woNumber, w.title, w.desc, assetIds[w.assetIdx], assetsData[w.assetIdx].location,
        ...
        if (w.status !== 'OPEN') {
            db.run('INSERT INTO work_order_status_history (id, woId, fromStatus, toStatus, changedBy, notes, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)', [(0, utils_1.generateId)(), id, 'OPEN', 'ASSIGNED', userIds['supervisor@example.com'], 'Assigned to technician', createdAt]);
        }
        if (w.status === 'IN_PROGRESS' || w.status === 'CLOSED') {
            db.run('INSERT INTO work_order_status_history (id, woId, fromStatus, toStatus, changedBy, notes, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)', [(0, utils_1.generateId)(), id, 'ASSIGNED', 'IN_PROGRESS', userIds['technician@example.com'], 'Work started', createdAt]);
        }
        if (w.status === 'CLOSED') {
            db.run('INSERT INTO work_order_status_history (id, woId, fromStatus, toStatus, changedBy, notes, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)', [(0, utils_1.generateId)(), id, 'IN_PROGRESS', 'CLOSED', userIds['technician@example.com'], 'Work completed', now]);
        }
    }
    ...
    for (const cl of checklistItems) {
        db.run('INSERT INTO work_order_checklists (id, woId, title, required, completed) VALUES (?, ?, ?, 1, ?)', [(0, utils_1.generateId)(), woIds[0], cl, cl !== 'Document findings' ? 1 : 0]);
    }
    // Add spare parts to WO
    db.run('INSERT INTO work_order_spare_parts (id, woId, itemId, plannedQuantity, usedQuantity, unit, unitCost, totalCost) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [(0, utils_1.generateId)(), woIds[0], sparePartIds[0], 2, 1, 'PCS', 850000, 850000]);
    db.run('INSERT INTO work_order_spare_parts (id, woId, itemId, plannedQuantity, usedQuantity, unit, unitCost, totalCost) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [(0, utils_1.generateId)(), woIds[0], sparePartIds[4], 10, 5, 'METER', 3500, 17500]);
    ...
    for (let i = 0; i < pmData.length; i++) {
        ...
        db.run(`INSERT INTO preventive_maintenance (id, assetId, title, description, frequency, startDate, nextDueDate, meterType, meterThreshold, assignedToId, status, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'ACTIVE', ?, ?)`, [id, assetIds[p.assetIdx], p.title, p.desc, p.freq, startDate, nextDue, p.meterType, p.meterThreshold,
        ...
        for (const item of pmChecklistItems) {
            db.run('INSERT INTO pm_checklists (id, pmId, title, required, createdAt) VALUES (?, ?, ?, 1, ?)', [(0, utils_1.generateId)(), id, item, now]);
        }
    }
    // ========== INVENTORY TRANSACTIONS ==========
    db.run('INSERT INTO inventory_transactions (id, itemId, warehouseId, transactionType, quantity, unitCost, referenceType, referenceId, notes, createdBy, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [(0, utils_1.generateId)(), sparePartIds[0], whId, 'IN', 20, 850000, null, null, 'Initial stock', userIds['admin@example.com'], now]);
    db.run('INSERT INTO inventory_transactions (id, itemId, warehouseId, transactionType, quantity, unitCost, referenceType, referenceId, notes, createdBy, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [(0, utils_1.generateId)(), sparePartIds[0], whId, 'OUT', 8, 850000, 'WORK_ORDER', woIds[0], 'WO-2026-000001 usage', userIds['technician@example.com'], now]);
    // ========== NOTIFICATIONS ==========
    db.run('INSERT INTO notifications (id, userId, type, title, message, referenceType, referenceId, read, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?)', [(0, utils_1.generateId)(), userIds['technician@example.com'], 'WO_ASSIGNED', 'Work Order Assigned', 'WO-2026-000003 (Compressor Pressure Drop) telah ditugaskan kepada Anda', 'WORK_ORDER', woIds[2], now]);
    db.run('INSERT INTO notifications (id, userId, type, title, message, referenceType, referenceId, read, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?)', [(0, utils_1.generateId)(), userIds['admin@example.com'], 'LOW_STOCK', 'Low Stock Alert', 'V-Belt B68 (SP-003) sudah di bawah stok minimum', 'SPARE_PART', sparePartIds[2], now]);
    db.run('INSERT INTO notifications (id, userId, type, title, message, referenceType, referenceId, read, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?)', [(0, utils_1.generateId)(), userIds['supervisor@example.com'], 'WO_CLOSED', 'Work Order Closed', 'WO-2026-000001 (CNC Spindle Vibration) telah selesai', 'WORK_ORDER', woIds[0], now]);
    // ========== AUDIT LOGS ==========
    db.run('INSERT INTO audit_logs (id, userId, action, entity, entityId, newValue, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)', [(0, utils_1.generateId)(), userIds['admin@example.com'], 'LOGIN', 'user', userIds['admin@example.com'], '{"email":"admin@example.com"}', now]);
