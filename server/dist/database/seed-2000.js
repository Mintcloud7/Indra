...
function randomId() { return (0, utils_1.generateId)(); }
...
async function seed2000() {
    ...
    for (const p of supervisorPerms) {
        if (permIds[p])
            db.run('INSERT INTO role_permissions (id, roleId, permissionId) VALUES (?, ?, ?)', [randomId(), roleIds['SUPERVISOR'], permIds[p]]);
    }
    ...
    for (const p of techPerms) {
        if (permIds[p])
            db.run('INSERT INTO role_permissions (id, roleId, permissionId) VALUES (?, ?, ?)', [randomId(), roleIds['TECHNICIAN'], permIds[p]]);
    }
    ...
    for (const p of managerPerms) {
        if (permIds[p])
            db.run('INSERT INTO role_permissions (id, roleId, permissionId) VALUES (?, ?, ?)', [randomId(), roleIds['MANAGER'], permIds[p]]);
    }
    // ========== USERS (10) ==========
    ...
    for (const sp of spDefs) {
        ...
        const min = randomInt(2, 10);
        const max = randomInt(30, 100);
        const whId = whIds[randomInt(0, whIds.length - 1)];
        db.run(`INSERT INTO spare_parts (id, itemCode, itemName, category, specification, unit, warehouseId, currentStock, minimumStock, maximumStock, unitCost, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [id, sp.code, sp.name, sp.cat, sp.spec, sp.unit, whId, stock, min, max, sp.cost, now, now]);
    }
    ...
    for (let i = 0; i < 25; i++) {
        ...
        db.run(`INSERT INTO assets (id, assetCode, assetName, assetType, location, serialNumber, manufacturer, model, purchaseDate, warrantyStart, warrantyEnd, status, description, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [id, `${prefix}-${assetCodeNums[i]}`, `${name} #${i + 1}`, type, loc, `${mfr}-SN-${randomInt(10000, 99999)}`, mfr, `Model-${randomInt(100, 999)}`, purchaseDate, purchaseDate, `${parseInt(purchaseDate.substring(0, 4)) + 3}-12-31`, randomItem(assetStatuses), `${name} at ${loc}`, now, now]);
    }
    ...
    for (let i = 0; i < 30; i++) {
        ...
        db.run('INSERT INTO asset_meter_readings (id, meterId, assetId, value, readingDate, recordedBy) VALUES (?, ?, ?, ?, ?, ?)', [randomId(), meterId, assetIds[assetIdx], mType === 'Temperature' ? randomInt(20, 80) : randomInt(100, 10000), randomDate(30), randomItem(techIds)]);
    }
    ...
    for (let i = 0; i < 1500; i++) {
        const id = randomId();
        woIds.push(id);
        const woNum = `WO-2026-${String(i + 1).padStart(6, '0')}`;
        ...
        if (['IN_PROGRESS', 'ON_HOLD', 'CLOSED'].includes(status))
            startedAt = randomDate(300);
        if (status === 'CLOSED') {
            closedAt = randomDate(200);
            completedAt = closedAt;
        }
        db.run(`INSERT INTO work_orders (id, woNumber, title, description, assetId, location, reportedById, supervisorId, assignedToId, priority, status, dueDate, createdAt, problemDescription, rootCause, resolution, startedAt, closedAt, completedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [id, woNum, `${problem} - WO#${i + 1}`, `Maintenance request for asset. ${problem}`, assetId, `Location-${randomInt(1, 5)}`,
        ...
        if (status !== 'OPEN')
            db.run('INSERT INTO work_order_status_history (id, woId, fromStatus, toStatus, changedBy, notes, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)', [randomId(), id, 'OPEN', 'ASSIGNED', supAssigned, 'Assigned to technician', createdAt]);
        if (['IN_PROGRESS', 'ON_HOLD', 'CLOSED'].includes(status))
            db.run('INSERT INTO work_order_status_history (id, woId, fromStatus, toStatus, changedBy, notes, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)', [randomId(), id, 'ASSIGNED', 'IN_PROGRESS', techAssigned, 'Work started', startedAt]);
        if (status === 'ON_HOLD')
            db.run('INSERT INTO work_order_status_history (id, woId, fromStatus, toStatus, changedBy, notes, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)', [randomId(), id, 'IN_PROGRESS', 'ON_HOLD', techAssigned, 'Waiting for parts', startedAt]);
        if (status === 'CLOSED')
            db.run('INSERT INTO work_order_status_history (id, woId, fromStatus, toStatus, changedBy, notes, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)', [randomId(), id, 'IN_PROGRESS', 'CLOSED', techAssigned, 'Work completed', closedAt]);
    }
    ...
    for (const woId of woIds) {
        ...
        for (let j = 0; j < numChecklists; j++) {
            ...
            db.run('INSERT INTO work_order_checklists (id, woId, title, required, completed, completedBy, completedAt) VALUES (?, ?, ?, ?, ?, ?, ?)', [randomId(), woId, shuffled[j], Math.random() > 0.2 ? 1 : 0, completed ? 1 : 0, completed ? randomItem(techIds) : null, completed ? randomDate(200) : null]);
        }
    }
    ...
    for (let i = 0; i < 500; i++) {
        ...
        db.run('INSERT INTO work_order_spare_parts (id, woId, itemId, plannedQuantity, usedQuantity, unit, unitCost, totalCost) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [randomId(), woIds[woIdx], sparePartIds[spIdx], planned, used, sp.unit, sp.cost, used * sp.cost]);
    }
    ...
    for (let i = 0; i < 30; i++) {
        ...
        db.run(`INSERT INTO preventive_maintenance (id, assetId, title, description, frequency, startDate, nextDueDate, meterType, meterThreshold, assignedToId, status, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [id, randomItem(assetIds), `${randomItem(pmTitles)} #${i + 1}`, `Scheduled preventive maintenance`, freq, startDate, nextDue,
    ...
    for (let i = 0; i < 800; i++) {
        ...
        db.run('INSERT INTO inventory_transactions (id, itemId, warehouseId, transactionType, quantity, unitCost, referenceType, referenceId, notes, createdBy, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [randomId(), sparePartIds[spIdx], whIds[randomInt(0, whIds.length - 1)], type, qty, spDefs[spIdx].cost,
    ...
    for (let i = 0; i < 100; i++) {
        const spIdx = randomInt(0, sparePartIds.length - 1);
        const sp = spDefs[spIdx];
        db.run('INSERT INTO purchase_requisitions (id, itemId, quantity, unit, reason, currentStock, minimumStock, status, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [randomId(), sparePartIds[spIdx], randomInt(5, 50), sp.unit, randomItem(['Low stock alert', 'Scheduled maintenance', 'Emergency repair', 'Stock replenishment']),
    ...
    for (let i = 0; i < 500; i++) {
        ...
        db.run('INSERT INTO notifications (id, userId, type, title, message, referenceType, referenceId, read, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [randomId(), randomItem(userIds), type, `${type.replace('_', ' ')} #${i + 1}`, `Notification message #${i + 1}`,
    ...
    for (let i = 0; i < 300; i++) {
        db.run('INSERT INTO audit_logs (id, userId, action, entity, entityId, newValue, ipAddress, userAgent, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [randomId(), randomItem(userIds), randomItem(auditActions), randomItem(['user', 'work_order', 'asset', 'spare_part', 'preventive_maintenance']),
            randomId(), `{"action":"test"}`, `192.168.1.${randomInt(1, 254)}`, 'Mozilla/5.0', randomDate(365)]);
    }
    console.log('Audit Logs: 300');
    // ========== INTEGRATION JOBS (50) ==========
    for (let i = 0; i < 50; i++) {
        db.run('INSERT INTO integration_jobs (id, type, referenceType, referenceId, payload, status, attempts, maxAttempts, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [randomId(), randomItem(['WO_CLOSE', 'PR_SYNC', 'INVENTORY_SYNC']), 'WORK_ORDER', woIds[randomInt(0, woIds.length - 1)],
