...
export class DashboardService {
  async getKpi(): Promise<any> {
    ...
    const openResult = await db.exec("SELECT COUNT(*) as c FROM work_orders WHERE status = 'OPEN'");
    const openWO = (openResult[0]?.values[0]?.[0] as number) || 0;

    const inProgressResult = await db.exec("SELECT COUNT(*) as c FROM work_orders WHERE status = 'IN_PROGRESS'");
    const inProgressWO = (inProgressResult[0]?.values[0]?.[0] as number) || 0;

    const onHoldResult = await db.exec("SELECT COUNT(*) as c FROM work_orders WHERE status = 'ON_HOLD'");
    const onHoldWO = (onHoldResult[0]?.values[0]?.[0] as number) || 0;

    const closedResult = await db.exec("SELECT COUNT(*) as c FROM work_orders WHERE status = 'CLOSED'");
    const closedWO = (closedResult[0]?.values[0]?.[0] as number) || 0;

    const today = new Date().toISOString().split('T')[0];
    const pmDueResult = await db.exec(
    ...
    const pmOverdueResult = await db.exec(
    ...
    const lowStockResult = await db.exec(
    ...
    const costResult = await db.exec(
    ...
    const downtimeResult = await db.exec(
    ...
    const mtbfResult = await db.exec(
  ...
  async getCharts(): Promise<any> {
    ...
    const woByStatusResult = await db.exec("SELECT status, COUNT(*) as count FROM work_orders GROUP BY status");
    const woByStatus = formatRows(woByStatusResult);

    const woByPriorityResult = await db.exec("SELECT priority, COUNT(*) as count FROM work_orders GROUP BY priority");
    const woByPriority = formatRows(woByPriorityResult);

    const maintenanceTrendResult = await db.exec(
    ...
    const sparePartUsageResult = await db.exec(
    ...
    const maintenanceCostTrendResult = await db.exec(
    ...
    const assetFailureTrendResult = await db.exec(
