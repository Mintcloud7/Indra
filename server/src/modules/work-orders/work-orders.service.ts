...
export class WorkOrderService {
  ...
  async start(id: string, userId: string): Promise<any> {
    ...
    db.run("UPDATE work_orders SET status = 'IN_PROGRESS', startedAt = ? WHERE id = ?", [nowISO(), id]);
    db.run(
  ...
  async close(id: string, userId: string, data: any): Promise<any> {
    ...
    const incompleteChecklistsResult = db.exec(
  ...
  async deleteChecklist(woId: string, checklistId: string): Promise<void> {
    const db = await getDb();
    const existing = db.exec("SELECT id FROM work_order_checklists WHERE id = ? AND woId = ?", [checklistId, woId]);
    if (!existing[0] || !existing[0].values.length) {
      throw new NotFoundError('Checklist item not found');
    }
    db.run("DELETE FROM work_order_checklists WHERE id = ? AND woId = ?", [checklistId, woId]);
    saveDb();
  }
  ...
  async deleteAttachment(woId: string, attachmentId: string): Promise<void> {
    const db = await getDb();
    const existing = db.exec("SELECT id FROM work_order_attachments WHERE id = ? AND woId = ?", [attachmentId, woId]);
    if (!existing[0] || !existing[0].values.length) {
      throw new NotFoundError('Attachment not found');
    }
    db.run("DELETE FROM work_order_attachments WHERE id = ? AND woId = ?", [attachmentId, woId]);
    saveDb();
  }
  ...
  async updateSparePart(woId: string, sparePartId: string, data: any): Promise<any> {
    ...
    if (data.usedQuantity !== undefined) {
      db.run("UPDATE work_order_spare_parts SET usedQuantity = ?, totalCost = ? * unitCost WHERE id = ? AND woId = ?",
        [data.usedQuantity, data.usedQuantity, sparePartId, woId]);
    }
    if (data.plannedQuantity !== undefined) {
      db.run("UPDATE work_order_spare_parts SET plannedQuantity = ? WHERE id = ? AND woId = ?",
